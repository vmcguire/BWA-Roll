# BWA-Roll — Read this before doing anything
...
can clone, browse, and reason about BWA-Roll on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-Roll happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/roll/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-Roll.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-Roll," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-Roll.vst3`
...
- **Category:** B (Generative / Rhythmic)
- **Tech tree tier:** Tier 5
- **Role:** Tension-ramp generator. Reads shared/groove/ + shared/emotion/.
- **Pain attacked:** Generative tension that lands on the human's groove, shaped by the song's emotion. Industry default: snap to metronome, ignore emotion.
- **Scope verdict:** per-track (per-instance shape decisions)
- **Band tier behavior:** Spectral-lift resolution scales with band tier.
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-Roll. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/roll/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-Roll row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-Roll.vst3` that:
...
```bash
git subtree push --prefix=plugins/roll \
  git@github.com:vmcguire/BWA-Roll.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-Roll.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-Roll._
